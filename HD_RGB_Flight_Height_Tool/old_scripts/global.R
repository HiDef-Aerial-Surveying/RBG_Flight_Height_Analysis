...
LIBS <- c("shiny","shinydashboard","rhandsontable","tidyverse","magrittr","shinyBS",
          ...
          "gmodels","ggthemes","readxl","HTSSIP","hablar",
          "Cairo","filesstrings","shinyFiles","doSNOW")
...
Run_Flight_Height <- function(dat,od,spp){
  dat <- data.frame(dat)
  ...
  opath <- paste0(od,"/",spp,"_Flight_heights_summary.csv")
  dpath <- paste0(od,"/",spp,"_Flight_heights_calculated.csv")
  write.csv(out_df,opath,row.names=F)
  write.csv(data_mod,dpath,row.names=F)
