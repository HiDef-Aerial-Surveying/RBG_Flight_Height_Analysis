...
SSpath <- "R:\\Projects\\HP00000 - Projects\\HP00124 - Energia North Celtic Sea\\500 - Data Processing\\2020 - Month 11 - Survey 01\\Zone107_M11_S01_D02_20"
###################################
...
OBpath <- "R:\\Projects\\HP00000 - Projects\\HP00124 - Energia North Celtic Sea\\500 - Data Processing\\2020 - Month 11 - Survey 01\\Output\\Clipping Obs" # made the unzipped for clipping folder to put observation sheets
###################################
## TYPE IN THE FULL PATH TO THE BOUNDARY SHAPEFILE
boundary.shapefile <- raster::shapefile("R:\\Projects\\HP00000 - Projects\\HP00124 - Energia North Celtic Sea\\100 - Project Control\\Survey Design\\Shapefiles\\Buildable Area Rev1_region.shp")
...
dir.create(paste0(pathname[1], "_clipped"))
 ...
 for(p in 1:length(mlist)) {
  ...
  SppDat <- inner_join(datout, obsout[, c("Camera", "Reel Name", "Frame", "Species", "id", "lat", "lon")]) # need to include date here because sometimes there was more than one survey date.
