...
all.frame %>% nest(data=c(out)) %>%
  ...
  ) %>% group_by(loc,Species,yr,month,year,category) %>% 
  summarise(ss = ss, 
            mn = round(mn, 1), 
...
Bplot <- all.frame %>% filter(Species == SPP[i]) %>%
 ggplot() +
  ...
  scale_fill_manual(values= rep("grey60", length(unique(all.frame$month))))+
  ggthemes::theme_gdocs()+
  ylab("Height (m)")+xlab("")+
  facet_wrap(month ~ loc, ncol = 3) +
  theme(
...
SppDatSPPloc2 <- SppDatSPP[which(SppDatSPP$loc == "SG2"),]
SppDatSPPloc3 <- SppDatSPP[which(SppDatSPP$loc == "SG3"),]

Dat.plot <- plot.fhs(SPP[i],SppDatSPPloc3,turbine.low,turbine.high,turbine.max)
Dat.plot
