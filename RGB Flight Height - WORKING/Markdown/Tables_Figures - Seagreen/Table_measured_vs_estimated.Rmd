...
MeasureDat1 <- MeasureDat %>% filter(yr == yrs[k], loc == locs[j])
SppDat1 <- SppDat %>% filter(yr == yrs[k], loc == locs[j])

MeasureDat1 %>% nest(-Species, -month, -year, -yr) -> aa

SppDat1 %>% nest(-Species, -month, -year, -yr) -> bb
...
for (l in 1:dim(cc)[1]){
cc$data.y1[i] <- unlist(ifelse(is.null(cc$data.y[[i]]), 0, nrow(cc$data.y[[l]])))
}
...
names(cc) <- c("Species", "month", "year", "measured", "estimated", "percm", "n", "enough")

data.table <- as.data.frame(cc)

names(data.table) <- c("Species", "Month", "Year", "Measured birds", "Est FH", "% estimated", "n", "Sufficient?")

caption_table <- paste0("Total number of flying ", i, " measured and estimated within the survey area in year ", yrs[k], " inclusive at ", locs[j], ".")

i_sp <- gsub(" ", "_", i)

ft <- data.table %>% filter(Species == i) %>% select(-"Species")
...
cat(knit_print(flextable(ft) %>% 
  ...
  set_caption(caption = paste0("Total number of flying ", i, " measured and estimated within the survey area in year ", yrs[k], " inclusive at ", locs[j], "."), 
                      style = "Table Caption",
                      autonum = run_autonum(seq_id = "tab", bkm = paste0("m_v_e_",i_sp)))))
 }
}
```