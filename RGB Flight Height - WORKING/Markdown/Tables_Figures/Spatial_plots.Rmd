...
leg_dat <- dplyr::filter(SppDat, Species == i, yr == yrs[k])

for(m in 1:length(unique(SppDat$mon.var))){

SppDatSPP <- dplyr::filter(SppDat, Species == i, mon.var == unique(SppDat$mon.var)[m], yr == yrs[k])
...
spatialplot <- ggplot() +  
  ...
  coord_equal() +
  ggthemes::theme_map()+
  ggtitle(unique(SppDat$mon.var)[m]) +
...
```
```{r make_spatialplot, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis', verbose = FALSE, fig.height = 4.5, fig.width = 10}


plot_grid(ggarrange(plot.list[[1]], plot.list[[2]], plot.list[[3]], plot.list[[4]], plot.list[[5]], plot.list[[6]], plot.list[[7]], plot.list[[8]], plot.list[[9]], plot.list[[10]], plot.list[[11]], plot.list[[12]], plot.list[[13]], plot.list[[14]], plot.list[[15]], plot.list[[16]],ncol = 4, nrow = 4), test_leg, ncol = 2, rel_widths = c(1, 0.1))

fig_num <-  run_autonum(seq_id = "Figure", pre_label = "Figure ", bkm = paste0("spatialplot_", i_sp, k))
  
cat(knit_print(block_caption(label = paste0("Two-dimensional spatial variation in estimated mean flight heights of ", i, " in year ", k, ". Flight height estimates were derived using an inverse distance weighted interpolation. Grey plots indicate months were no birds of this species were recorded."), 
